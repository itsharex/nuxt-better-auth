...
export default defineNuxtConfig({
  ...
  betterAuth: {
    ...
    // secondaryStorage: 'session' // Optional: use session storage instead of cookies
  }
})
```
...
::field-group
  ...
  ::field{name="appName" type="string"}
    Application name used in emails and OAuth consent screens. Passed directly to Better Auth.
  ::
::
...
export default defineServerAuth(({ db, runtimeConfig }) => ({
  // Use the database connection provided by the module
  database: drizzleAdapter(db),

  // Access runtime config for OAuth credentials
  socialProviders: {
    github: {
